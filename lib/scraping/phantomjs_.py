...
class PhantomJS_(webdriver.PhantomJS):
	...
	def find_elements_with_handling_exceptions(self, by="XPATH", tag="", warning_messages=True, timeout=30, url=""):
		##By: CLASS_NAME, CSS_SELECTOR, ID, LINK_TEXT
		##, NAME, PARTIAL_LINK_TEXT,TAG_NAME, XPATH
		self.log.debug(__class__.__name__ + "." + sys._getframe().f_code.co_name + " start.")
		self.log.debug("by: " + by + " , + tag: " + tag)
	...
	def convert_svg_to_png(self, svg_path):
		self.log.debug(__class__.__name__ + "." + sys._getframe().f_code.co_name + " start")
		#self.get("file://" + svg_path)
		self.get("file:///home/alladmin/f/program/country_graph/data/flags/Algeria.svg")
		self.save_current_page("../../data/flags/Algeria.png")
		png_path = ""
		self.log.debug(__class__.__name__ + "." + sys._getframe().f_code.co_name + " finished. return " + png_path)
